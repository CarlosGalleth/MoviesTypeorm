...
export const listMoviesService = async (page: any, perPage: any) => {
  ...
  const prevPage: string | null = skip - 1 === 0 ? null : `http://localhost:3000/movies?page=${skip - 1}&perPage=${take}`
  const nextPage: string | null = skip === pages || skip > pages ? null : `http://localhost:3000/movies?page=${skip + 1}&perPage=${take}`
